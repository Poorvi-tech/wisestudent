...
const SMART_SPENDING_CHECKPOINT_STAGES = [
  {
    id: 1,
...
const totalStages = SMART_SPENDING_CHECKPOINT_STAGES.length;
...
const SmartSpendingCheckpoint = () => {
  ...
  const handleChoice = (option) => {
    ...
    resetFeedback();
    const currentStageData = SMART_SPENDING_CHECKPOINT_STAGES[currentStage];
  ...
  const stage = SMART_SPENDING_CHECKPOINT_STAGES[Math.min(currentStage, totalStages - 1)];
  ...
  return (
    <GameShell
      ...
      totalXp={totalXp}
      maxScore={SMART_SPENDING_CHECKPOINT_STAGES.length}
      currentLevel={Math.min(currentStage + 1, SMART_SPENDING_CHECKPOINT_STAGES.length)}
      totalLevels={SMART_SPENDING_CHECKPOINT_STAGES.length}
      gameId={gameId}
      gameType="finance"
                ...
                <div className="mt-4 flex justify-center">
                  <button
                    onClick={() => {
                      const updatedHistory = [
                        ...history,
                        { stageId: SMART_SPENDING_CHECKPOINT_STAGES[currentStage].id, isCorrect: SMART_SPENDING_CHECKPOINT_STAGES[currentStage].options.find(opt => opt.id === selectedOption)?.isCorrect },
                      ];
                      const correctCount = updatedHistory.filter((item) => item.isCorrect).length;
                      const passed = correctCount === successThreshold;
                      setFinalScore(correctCount);
              ...
              {showResult && (
                <>
                  ...
                  <p className="text-sm text-white/70">
                    Skill unlocked: <strong>Smart spending mastery</strong>
                  </p>
