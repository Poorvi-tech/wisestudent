...
const MONTHLY_BUDGET_ORDER_STAGES = [
  {
    id: 1,
...
const totalStages = MONTHLY_BUDGET_ORDER_STAGES.length;
...
const MonthlyBudgetOrder = () => {
  ...
  const handleChoice = (option) => {
    ...
    resetFeedback();
    const currentStageData = MONTHLY_BUDGET_ORDER_STAGES[currentStage];
  ...
  const stage = MONTHLY_BUDGET_ORDER_STAGES[Math.min(currentStage, totalStages - 1)];
  ...
  return (
    <GameShell
      ...
      totalXp={totalXp}
      maxScore={MONTHLY_BUDGET_ORDER_STAGES.length}
      currentLevel={Math.min(currentStage + 1, MONTHLY_BUDGET_ORDER_STAGES.length)}
      totalLevels={MONTHLY_BUDGET_ORDER_STAGES.length}
      gameId={gameId}
      gameType="finance"
                ...
                <div className="mt-4 flex justify-center">
                  <button
                    onClick={() => {
                      const updatedHistory = [
                        ...history,
                        { stageId: MONTHLY_BUDGET_ORDER_STAGES[currentStage].id, isCorrect: MONTHLY_BUDGET_ORDER_STAGES[currentStage].options.find(opt => opt.id === selectedOption)?.isCorrect },
                      ];
                      const correctCount = updatedHistory.filter((item) => item.isCorrect).length;
                      const passed = correctCount === successThreshold;
                      setFinalScore(correctCount);
              ...
              {showResult && (
                <>
                  ...
                  <p className="text-sm text-white/70">
                    Skill unlocked: <strong>Budget prioritization</strong>
                  </p>
                  {!hasPassed && (
