...
const BORROWING_TO_SPEND_STAGES = [
  {
    id: 1,
...
const totalStages = BORROWING_TO_SPEND_STAGES.length;
...
const BorrowingToSpend = () => {
  ...
  const handleChoice = (option) => {
    ...
    resetFeedback();
    const currentStageData = BORROWING_TO_SPEND_STAGES[currentStage];
  ...
  const stage = BORROWING_TO_SPEND_STAGES[Math.min(currentStage, totalStages - 1)];
  ...
  return (
    <GameShell
      ...
      totalXp={totalXp}
      maxScore={BORROWING_TO_SPEND_STAGES.length}
      currentLevel={Math.min(currentStage + 1, BORROWING_TO_SPEND_STAGES.length)}
      totalLevels={BORROWING_TO_SPEND_STAGES.length}
      gameId={gameId}
      gameType="finance"
                ...
                <div className="mt-4 flex justify-center">
                  <button
                    onClick={() => {
                      const updatedHistory = [
                        ...history,
                        { stageId: BORROWING_TO_SPEND_STAGES[currentStage].id, isCorrect: BORROWING_TO_SPEND_STAGES[currentStage].options.find(opt => opt.id === selectedOption)?.isCorrect },
                      ];
                      const correctCount = updatedHistory.filter((item) => item.isCorrect).length;
                      const passed = correctCount === successThreshold;
                      setFinalScore(correctCount);
              ...
              {showResult && (
                <>
                  ...
                  <p className="text-sm text-white/70">
                    Skill unlocked: <strong>Debt-free spending awareness</strong>
                  </p>
                  {!hasPassed && (
