...
const totalStages = SALE_PSYCHOLOGY_STAGES.length;
...
const SalePsychology = () => {
  ...
  const handleChoice = (option) => {
    ...
    resetFeedback();
    const currentStageData = SALE_PSYCHOLOGY_STAGES[currentStage];
  ...
  const stage = SALE_PSYCHOLOGY_STAGES[Math.min(currentStage, totalStages - 1)];
  ...
  return (
    <GameShell
      ...
      totalXp={totalXp}
      maxScore={SALE_PSYCHOLOGY_STAGES.length}
      currentLevel={Math.min(currentStage + 1, SALE_PSYCHOLOGY_STAGES.length)}
      totalLevels={SALE_PSYCHOLOGY_STAGES.length}
      gameId={gameId}
      gameType="finance"
                ...
                <div className="mt-4 flex justify-center">
                  <button
                    onClick={() => {
                      const updatedHistory = [
                        ...history,
                        { stageId: SALE_PSYCHOLOGY_STAGES[currentStage].id, isCorrect: SALE_PSYCHOLOGY_STAGES[currentStage].options.find(opt => opt.id === selectedOption)?.isCorrect },
                      ];
                      const correctCount = updatedHistory.filter((item) => item.isCorrect).length;
                      const passed = correctCount === successThreshold;
                      setFinalScore(correctCount);
              ...
              {showResult && (
                <>
                  ...
                  <p className="text-sm text-white/70">
                    Skill unlocked: <strong>Sale psychology awareness</strong>
                  </p>
                  {!hasPassed && (
