...
const totalStages = EMERGENCY_LINE_STAGES.length;
...
const EmergencyLineInBudget = () => {
  ...
  const handleChoice = (option) => {
    ...
    resetFeedback();
    const currentStageData = EMERGENCY_LINE_STAGES[currentStage];
  ...
  const stage = EMERGENCY_LINE_STAGES[Math.min(currentStage, totalStages - 1)];
  ...
  return (
    <GameShell
      ...
      totalXp={totalXp}
      maxScore={EMERGENCY_LINE_STAGES.length}
      currentLevel={Math.min(currentStage + 1, EMERGENCY_LINE_STAGES.length)}
      totalLevels={EMERGENCY_LINE_STAGES.length}
      gameId={gameId}
      gameType="finance"
                ...
                <div className="mt-4 flex justify-center">
                  <button
                    onClick={() => {
                      const updatedHistory = [
                        ...history,
                        { stageId: EMERGENCY_LINE_STAGES[currentStage].id, isCorrect: EMERGENCY_LINE_STAGES[currentStage].options.find(opt => opt.id === selectedOption)?.isCorrect },
                      ];
                      const correctCount = updatedHistory.filter((item) => item.isCorrect).length;
                      const passed = correctCount === successThreshold;
                      setFinalScore(correctCount);
