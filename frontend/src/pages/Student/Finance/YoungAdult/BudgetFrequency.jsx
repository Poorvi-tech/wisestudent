...
const totalStages = BUDGET_FREQUENCY_STAGES.length;
...
const BudgetFrequency = () => {
  ...
  const handleChoice = (option) => {
    ...
    resetFeedback();
    const currentStageData = BUDGET_FREQUENCY_STAGES[currentStage];
  ...
  const stage = BUDGET_FREQUENCY_STAGES[Math.min(currentStage, totalStages - 1)];
  ...
  return (
    <GameShell
      ...
      totalXp={totalXp}
      maxScore={BUDGET_FREQUENCY_STAGES.length}
      currentLevel={Math.min(currentStage + 1, BUDGET_FREQUENCY_STAGES.length)}
      totalLevels={BUDGET_FREQUENCY_STAGES.length}
      gameId={gameId}
      gameType="finance"
                ...
                <div className="mt-4 flex justify-center">
                  <button
                    onClick={() => {
                      const updatedHistory = [
                        ...history,
                        { stageId: BUDGET_FREQUENCY_STAGES[currentStage].id, isCorrect: BUDGET_FREQUENCY_STAGES[currentStage].options.find(opt => opt.id === selectedOption)?.isCorrect },
                      ];
                      const correctCount = updatedHistory.filter((item) => item.isCorrect).length;
                      const passed = correctCount === successThreshold;
                      setFinalScore(correctCount);
              ...
              {showResult && (
                <>
                  ...
                  <p className="text-sm text-white/70">
                    Skill unlocked: <strong>Budget review discipline</strong>
                  </p>
                  {!hasPassed && (
