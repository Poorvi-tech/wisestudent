...
const totalStages = WEEKEND_SPENDING_STAGES.length;
...
const WeekendSpendingReality = () => {
  ...
  const handleChoice = (option) => {
    ...
    resetFeedback();
    const currentStageData = WEEKEND_SPENDING_STAGES[currentStage];
  ...
  const stage = WEEKEND_SPENDING_STAGES[Math.min(currentStage, totalStages - 1)];
  ...
  return (
    <GameShell
      ...
      totalXp={totalXp}
      maxScore={WEEKEND_SPENDING_STAGES.length}
      currentLevel={Math.min(currentStage + 1, WEEKEND_SPENDING_STAGES.length)}
      totalLevels={WEEKEND_SPENDING_STAGES.length}
      gameId={gameId}
      gameType="finance"
                ...
                <div className="mt-4 flex justify-center">
                  <button
                    onClick={() => {
                      const updatedHistory = [
                        ...history,
                        { stageId: WEEKEND_SPENDING_STAGES[currentStage].id, isCorrect: WEEKEND_SPENDING_STAGES[currentStage].options.find(opt => opt.id === selectedOption)?.isCorrect },
                      ];
                      const correctCount = updatedHistory.filter((item) => item.isCorrect).length;
                      const passed = correctCount === successThreshold;
                      setFinalScore(correctCount);
              ...
              {showResult && (
                <>
                  ...
                  <p className="text-sm text-white/70">
                    Skill unlocked: <strong>Weekend spending awareness</strong>
                  </p>
                  {!hasPassed && (
