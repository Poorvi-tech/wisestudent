...
const CONVENIENCE_SPENDING_STAGES = [
  {
    id: 1,
...
const totalStages = CONVENIENCE_SPENDING_STAGES.length;
...
const ConvenienceSpending = () => {
  ...
  const handleChoice = (option) => {
    ...
    resetFeedback();
    const currentStageData = CONVENIENCE_SPENDING_STAGES[currentStage];
  ...
  const stage = CONVENIENCE_SPENDING_STAGES[Math.min(currentStage, totalStages - 1)];
  ...
  return (
    <GameShell
      ...
      totalXp={totalXp}
      maxScore={CONVENIENCE_SPENDING_STAGES.length}
      currentLevel={Math.min(currentStage + 1, CONVENIENCE_SPENDING_STAGES.length)}
      totalLevels={CONVENIENCE_SPENDING_STAGES.length}
      gameId={gameId}
      gameType="finance"
                ...
                <div className="mt-4 flex justify-center">
                  <button
                    onClick={() => {
                      const updatedHistory = [
                        ...history,
                        { stageId: CONVENIENCE_SPENDING_STAGES[currentStage].id, isCorrect: CONVENIENCE_SPENDING_STAGES[currentStage].options.find(opt => opt.id === selectedOption)?.isCorrect },
                      ];
                      const correctCount = updatedHistory.filter((item) => item.isCorrect).length;
                      const passed = correctCount === successThreshold;
                      setFinalScore(correctCount);
              ...
              {showResult && (
                <>
                  ...
                  <p className="text-sm text-white/70">
                    Skill unlocked: <strong>Convenience cost awareness</strong>
                  </p>
                  {!hasPassed && (
