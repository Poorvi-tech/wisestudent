...
const TRACKING_LIFESTYLE_COSTS_STAGES = [
  {
    id: 1,
...
const totalStages = TRACKING_LIFESTYLE_COSTS_STAGES.length;
...
const TrackingLifestyleCosts = () => {
  ...
  const handleChoice = (option) => {
    ...
    resetFeedback();
    const currentStageData = TRACKING_LIFESTYLE_COSTS_STAGES[currentStage];
  ...
  const stage = TRACKING_LIFESTYLE_COSTS_STAGES[Math.min(currentStage, totalStages - 1)];
  ...
  return (
    <GameShell
      ...
      totalXp={totalXp}
      maxScore={TRACKING_LIFESTYLE_COSTS_STAGES.length}
      currentLevel={Math.min(currentStage + 1, TRACKING_LIFESTYLE_COSTS_STAGES.length)}
      totalLevels={TRACKING_LIFESTYLE_COSTS_STAGES.length}
      gameId={gameId}
      gameType="finance"
                ...
                <div className="mt-4 flex justify-center">
                  <button
                    onClick={() => {
                      const updatedHistory = [
                        ...history,
                        { stageId: TRACKING_LIFESTYLE_COSTS_STAGES[currentStage].id, isCorrect: TRACKING_LIFESTYLE_COSTS_STAGES[currentStage].options.find(opt => opt.id === selectedOption)?.isCorrect },
                      ];
                      const correctCount = updatedHistory.filter((item) => item.isCorrect).length;
                      const passed = correctCount === successThreshold;
                      setFinalScore(correctCount);
