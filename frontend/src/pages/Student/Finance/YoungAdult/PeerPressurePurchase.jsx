...
const PEER_PRESSURE_PURCHASE_STAGES = [
  {
    id: 1,
...
const totalStages = PEER_PRESSURE_PURCHASE_STAGES.length;
...
const PeerPressurePurchase = () => {
  ...
  const handleChoice = (option) => {
    ...
    resetFeedback();
    const currentStageData = PEER_PRESSURE_PURCHASE_STAGES[currentStage];
  ...
  const stage = PEER_PRESSURE_PURCHASE_STAGES[Math.min(currentStage, totalStages - 1)];
  ...
  return (
    <GameShell
      ...
      totalXp={totalXp}
      maxScore={PEER_PRESSURE_PURCHASE_STAGES.length}
      currentLevel={Math.min(currentStage + 1, PEER_PRESSURE_PURCHASE_STAGES.length)}
      totalLevels={PEER_PRESSURE_PURCHASE_STAGES.length}
      gameId={gameId}
      gameType="finance"
                ...
                <div className="mt-4 flex justify-center">
                  <button
                    onClick={() => {
                      const updatedHistory = [
                        ...history,
                        { stageId: PEER_PRESSURE_PURCHASE_STAGES[currentStage].id, isCorrect: PEER_PRESSURE_PURCHASE_STAGES[currentStage].options.find(opt => opt.id === selectedOption)?.isCorrect },
                      ];
                      const correctCount = updatedHistory.filter((item) => item.isCorrect).length;
                      const passed = correctCount === successThreshold;
                      setFinalScore(correctCount);
              ...
              {showResult && (
                <>
                  ...
                  <p className="text-sm text-white/70">
                    Skill unlocked: <strong>Peer pressure resistance</strong>
                  </p>
                  {!hasPassed && (
