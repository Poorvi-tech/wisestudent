...
const totalStages = BORROWING_WANTS_STAGES.length;
...
const BorrowingForWants = () => {
  ...
  const handleChoice = (option) => {
    ...
    resetFeedback();
    const currentStageData = BORROWING_WANTS_STAGES[currentStage];
  ...
  const stage = BORROWING_WANTS_STAGES[Math.min(currentStage, totalStages - 1)];
  ...
  return (
    <GameShell
      ...
      totalXp={totalXp}
      maxScore={BORROWING_WANTS_STAGES.length}
      currentLevel={Math.min(currentStage + 1, BORROWING_WANTS_STAGES.length)}
      totalLevels={BORROWING_WANTS_STAGES.length}
      gameId={gameId}
      gameType="finance"
                ...
                <div className="mt-4 flex justify-center">
                  <button
                    onClick={() => {
                      const updatedHistory = [
                        ...history,
                        { stageId: BORROWING_WANTS_STAGES[currentStage].id, isCorrect: BORROWING_WANTS_STAGES[currentStage].options.find(opt => opt.id === selectedOption)?.isCorrect },
                      ];
                      const correctCount = updatedHistory.filter((item) => item.isCorrect).length;
                      const passed = correctCount === successThreshold;
                      setFinalScore(correctCount);
