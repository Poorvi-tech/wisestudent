...
const LIFESTYLE_INFLATION_STAGES = [
  {
    id: 1,
...
const totalStages = LIFESTYLE_INFLATION_STAGES.length;
...
const LifestyleInflation = () => {
  ...
  const handleChoice = (option) => {
    ...
    resetFeedback();
    const currentStageData = LIFESTYLE_INFLATION_STAGES[currentStage];
  ...
  const stage = LIFESTYLE_INFLATION_STAGES[Math.min(currentStage, totalStages - 1)];
  ...
  return (
    <GameShell
      ...
      totalXp={totalXp}
      maxScore={LIFESTYLE_INFLATION_STAGES.length}
      currentLevel={Math.min(currentStage + 1, LIFESTYLE_INFLATION_STAGES.length)}
      totalLevels={LIFESTYLE_INFLATION_STAGES.length}
      gameId={gameId}
      gameType="finance"
                ...
                <div className="mt-4 flex justify-center">
                  <button
                    onClick={() => {
                      const updatedHistory = [
                        ...history,
                        { stageId: LIFESTYLE_INFLATION_STAGES[currentStage].id, isCorrect: LIFESTYLE_INFLATION_STAGES[currentStage].options.find(opt => opt.id === selectedOption)?.isCorrect },
                      ];
                      const correctCount = updatedHistory.filter((item) => item.isCorrect).length;
                      const passed = correctCount === successThreshold;
                      setFinalScore(correctCount);
              ...
              {showResult && (
                <>
                  ...
                  <p className="text-sm text-white/70">
                    Skill unlocked: <strong>Lifestyle inflation awareness</strong>
                  </p>
                  {!hasPassed && (
