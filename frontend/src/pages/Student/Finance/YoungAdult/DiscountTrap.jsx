...
const totalStages = DISCOUNT_TRAP_STAGES.length;
...
const DiscountTrap = () => {
  ...
  const handleChoice = (option) => {
    ...
    resetFeedback();
    const currentStageData = DISCOUNT_TRAP_STAGES[currentStage];
  ...
  const stage = DISCOUNT_TRAP_STAGES[Math.min(currentStage, totalStages - 1)];
  ...
  return (
    <GameShell
      ...
      totalXp={totalXp}
      maxScore={DISCOUNT_TRAP_STAGES.length}
      currentLevel={Math.min(currentStage + 1, DISCOUNT_TRAP_STAGES.length)}
      totalLevels={DISCOUNT_TRAP_STAGES.length}
      gameId={gameId}
      gameType="finance"
                ...
                <div className="mt-4 flex justify-center">
                  <button
                    onClick={() => {
                      const updatedHistory = [
                        ...history,
                        { stageId: DISCOUNT_TRAP_STAGES[currentStage].id, isCorrect: DISCOUNT_TRAP_STAGES[currentStage].options.find(opt => opt.id === selectedOption)?.isCorrect },
                      ];
                      const correctCount = updatedHistory.filter((item) => item.isCorrect).length;
                      const passed = correctCount === successThreshold;
                      setFinalScore(correctCount);
              ...
              {showResult && (
                <>
                  ...
                  <p className="text-sm text-white/70">
                    Skill unlocked: <strong>Discount evaluation mastery</strong>
                  </p>
                  {!hasPassed && (
