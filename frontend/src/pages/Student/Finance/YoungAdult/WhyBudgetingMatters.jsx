...
const WHY_BUDGETING_MATTERS_STAGES = [
  {
    id: 1,
...
const totalStages = WHY_BUDGETING_MATTERS_STAGES.length;
...
const WhyBudgetingMatters = () => {
  ...
  const handleChoice = (option) => {
    ...
    resetFeedback();
    const currentStageData = WHY_BUDGETING_MATTERS_STAGES[currentStage];
  ...
  const stage = WHY_BUDGETING_MATTERS_STAGES[Math.min(currentStage, totalStages - 1)];
  ...
  return (
    <GameShell
      ...
      totalXp={totalXp}
      maxScore={WHY_BUDGETING_MATTERS_STAGES.length}
      currentLevel={Math.min(currentStage + 1, WHY_BUDGETING_MATTERS_STAGES.length)}
      totalLevels={WHY_BUDGETING_MATTERS_STAGES.length}
      gameId={gameId}
      gameType="finance"
                ...
                <div className="mt-4 flex justify-center">
                  <button
                    onClick={() => {
                      const updatedHistory = [
                        ...history,
                        { stageId: WHY_BUDGETING_MATTERS_STAGES[currentStage].id, isCorrect: WHY_BUDGETING_MATTERS_STAGES[currentStage].options.find(opt => opt.id === selectedOption)?.isCorrect },
                      ];
                      const correctCount = updatedHistory.filter((item) => item.isCorrect).length;
                      const passed = correctCount === successThreshold;
                      setFinalScore(correctCount);
              ...
              {showResult && (
                <>
                  ...
                  <p className="text-sm text-white/70">
                    Skill unlocked: <strong>Budgeting fundamentals</strong>
                  </p>
                  {!hasPassed && (
