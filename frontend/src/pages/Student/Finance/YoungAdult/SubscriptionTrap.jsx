...
const SUBSCRIPTION_TRAP_STAGES = [
  {
    id: 1,
...
const totalStages = SUBSCRIPTION_TRAP_STAGES.length;
...
const SubscriptionTrap = () => {
  ...
  const handleChoice = (option) => {
    ...
    resetFeedback();
    const currentStageData = SUBSCRIPTION_TRAP_STAGES[currentStage];
  ...
  const stage = SUBSCRIPTION_TRAP_STAGES[Math.min(currentStage, totalStages - 1)];
  ...
  return (
    <GameShell
      ...
      totalXp={totalXp}
      maxScore={SUBSCRIPTION_TRAP_STAGES.length}
      currentLevel={Math.min(currentStage + 1, SUBSCRIPTION_TRAP_STAGES.length)}
      totalLevels={SUBSCRIPTION_TRAP_STAGES.length}
      gameId={gameId}
      gameType="finance"
                ...
                <div className="mt-4 flex justify-center">
                  <button
                    onClick={() => {
                      const updatedHistory = [
                        ...history,
                        { stageId: SUBSCRIPTION_TRAP_STAGES[currentStage].id, isCorrect: SUBSCRIPTION_TRAP_STAGES[currentStage].options.find(opt => opt.id === selectedOption)?.isCorrect },
                      ];
                      const correctCount = updatedHistory.filter((item) => item.isCorrect).length;
                      const passed = correctCount === successThreshold;
                      setFinalScore(correctCount);
              ...
              {showResult && (
                <>
                  ...
                  <p className="text-sm text-white/70">
                    Skill unlocked: <strong>Subscription management</strong>
                  </p>
                  {!hasPassed && (
