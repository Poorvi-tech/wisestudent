...
const LEFT_ITEMS = [
  { id: "shop-rent", text: "Shop Rent" },
  { id: "electricity", text: "Electricity Bill" },
  { id: "customer-payment", text: "Customer Payment" },
  { id: "stock-purchase", text: "Buying Inventory" },
  { id: "loan-received", text: "Bank Loan Disbursed" },
];

const RIGHT_ITEMS = [
  { id: "expense-stock", text: "Cost of Goods" },
  { id: "income-sales", text: "Sales Income" },
  { id: "expense-variable", text: "Variable Expense" },
  { id: "cash-inflow", text: "Non-Sales Inflow" },
  { id: "expense-fixed", text: "Fixed Expense" },
];

const SOLUTIONS = {
  "shop-rent": "expense-fixed",
  "electricity": "expense-variable",
  "customer-payment": "income-sales",
  "stock-purchase": "expense-stock",
  "loan-received": "cash-inflow",
};

const PuzzleExpenseSort = () => {
  ...
  const handleRightSelect = (r) => {
    ...
    if (r.id === correctRight) {
      ...
      setOutcomeText(`${selectedLeft.text} → ${r.text} matched successfully.`);
      const doneCount = Object.keys(matched).length + 1;
      if (doneCount === totalPairs) {
        setShowResult(true);
      }
    } else {
      setOutcomeText("Incorrect match. Think about where the money goes.");
    }
  ...
  return (
    <GameShell
      ...
      subtitle={
        showResult
          ? "Sorting complete! Keeping expenses and income organized is key."
          : `Match the transactions: ${score}/${totalPairs} matched`
      }
...
export default PuzzleExpenseSort;
