...
const LEFT_ITEMS = [
  { id: "bank-account", text: "Bank Account" },
  { id: "cash-box", text: "Cash Box" },
  { id: "digital-payment", text: "Digital Payment" },
  { id: "informal-loan", text: "Local Moneylender" },
  { id: "tax-return", text: "Tax Return" },
];

const RIGHT_ITEMS = [
  { id: "high-interest-trap", text: "High Interest Debt Trap" },
  { id: "instant-record", text: "Instant Record & Safe Transfer" },
  { id: "risk-no-trail", text: "High Risk & No Audit Trail" },
  { id: "interest-loans", text: "Earns Interest & Unlocks Loans" },
  { id: "formal-proof", text: "Formal Proof of Income" },
];

const SOLUTIONS = {
  "bank-account": "interest-loans",
  "cash-box": "risk-no-trail",
  "digital-payment": "instant-record",
  "informal-loan": "high-interest-trap",
  "tax-return": "formal-proof",
};

const PuzzleBankingBenefits = () => {
  ...
  const handleRightSelect = (r) => {
    ...
    if (r.id === correctRight) {
      ...
      setOutcomeText(`Correct! ${selectedLeft.text} → ${r.text}`);
      const doneCount = Object.keys(matched).length + 1;
      if (doneCount === totalPairs) {
        setShowResult(true);
      }
    } else {
      setOutcomeText("Incorrect match. Think about the specific benefit or risk.");
    }
  ...
  return (
    <GameShell
      ...
      subtitle={
        showResult
          ? "Puzzle complete! Formal banking unlocks growth and security."
          : `Match the concepts: ${score}/${totalPairs} matched`
      }
...
export default PuzzleBankingBenefits;
