...
const LEFT_ITEMS = [
  { id: "missing-cash", text: "Physical Cash box is ₹500 short" },
  { id: "missing-bank", text: "Bank Statement is ₹200 lower than app" },
  { id: "missing-profit", text: "App shows huge profit, but pockets are empty" },
  { id: "missing-supplier", text: "Supplier claims ₹5,000 outstanding debt" },
  { id: "missing-tax", text: "Tax liability seems overly high" },
];

const RIGHT_ITEMS = [
  { id: "deductible-expense", text: "Missed logging a Tax-Deductible Expense" },
  { id: "credit-sale", text: "Logged Credit Sale as 'Cash Received'" },
  { id: "bank-fee", text: "Forgot to log Bank Processing Fees" },
  { id: "owner-draw", text: "Forgot to log Owner Personal Withdrawal" },
  { id: "purchase-invoice", text: "Missed logging latest Purchase Invoice" },
];

const SOLUTIONS = {
  "missing-cash": "owner-draw",
  "missing-bank": "bank-fee",
  "missing-profit": "credit-sale",
  "missing-supplier": "purchase-invoice",
  "missing-tax": "deductible-expense",
};

const PuzzleTheMissingPiece = () => {
  ...
  const handleRightSelect = (r) => {
    ...
    if (r.id === correctRight) {
      ...
      setOutcomeText(`Audit success! ${selectedLeft.text} → ${r.text}`);
      const doneCount = Object.keys(matched).length + 1;
      if (doneCount === totalPairs) {
        setShowResult(true);
      }
    } else {
      setOutcomeText("Audit Error: That missing piece doesn't solve this mismatch.");
    }
  ...
  return (
    <GameShell
      ...
      subtitle={
        showResult
          ? "Puzzle complete! You successfully balanced the books."
          : `Find the missing pieces: ${score}/${totalPairs} matched`
      }
...
export default PuzzleTheMissingPiece;
